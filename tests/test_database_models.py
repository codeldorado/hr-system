...
def sample_payslip_data():
    ...
    return {
        ...
        "file_url": f"https://s3.amazonaws.com/bucket/payslips/123/2024/12/{unique_id}.pdf",
        "file_size": 1048576,
    }
...
class TestPayslipModel:
    ...
    def test_query_operations(self, db_session):
        ...
        employee_100_payslips = (
            db_session.query(Payslip).filter(Payslip.employee_id == 100).all()
        )
        assert len(employee_100_payslips) == 2

        # Test filtering by year
        year_2024_payslips = (
            db_session.query(Payslip).filter(Payslip.year == 2024).all()
        )
        assert len(year_2024_payslips) == 3

        # Test filtering by month
        jan_payslips = db_session.query(Payslip).filter(Payslip.month == 1).all()
        assert len(jan_payslips) == 2

        # Test combined filters
        specific_payslip = (
            db_session.query(Payslip)
            .filter(
                Payslip.employee_id == 100, Payslip.month == 2, Payslip.year == 2024
            )
            .first()
        )
    ...
    def test_ordering(self, db_session):
        ...
        payslips = (
            db_session.query(Payslip)
            .filter(Payslip.employee_id == 300)
            .order_by(Payslip.upload_timestamp.desc())
            .all()
        )
    ...
    def test_update_operations(self, db_session, sample_payslip_data):
        ...
        # Verify update
        updated_payslip = (
            db_session.query(Payslip).filter(Payslip.id == original_id).first()
        )

        assert updated_payslip.filename == "updated_filename.pdf"
        assert (
            updated_payslip.upload_timestamp == original_timestamp
        )  # Should not change
    ...
    def test_delete_operations(self, db_session, sample_payslip_data):
        ...
        # Verify deletion
        deleted_payslip = (
            db_session.query(Payslip).filter(Payslip.id == payslip_id).first()
        )
